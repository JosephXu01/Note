...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "me.wslong.ios.Note" in the application's documents directory.
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    if (![_persistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:storeURL options:nil error:&error]) {
        ...
        if (![fileManager removeItemAtPath:[NSString stringWithContentsOfURL:storeURL encoding:NSUTF8StringEncoding error:nil] error:&fileDeleteError]) {
            NSLog(@"unable to delete note.sqlite : %@",[fileDeleteError localizedDescription]);
        }
    }
